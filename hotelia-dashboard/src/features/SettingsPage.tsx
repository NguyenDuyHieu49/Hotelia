...
export function SettingsPage() {
  ...
  return <div className="max-w-xl space-y-6"><h1 className="text-2xl font-bold">Cài đặt tài khoản</h1>{message && <p role="status">{message}</p>}
    <form className="bg-white p-6 rounded space-y-4" onSubmit={async e=>{e.preventDefault();setBusy(true);try {await userApi.updateMe({name,phone});setMessage('Đã lưu hồ sơ');} catch(e){setMessage(errorText(e));} finally {setBusy(false);}}}>
